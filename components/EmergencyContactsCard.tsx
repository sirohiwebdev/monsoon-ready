...
export default function EmergencyContactsCard({ place, state, lang }: EmergencyContactsCardProps) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <h2 className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-slate-500">
        <ShieldAlert size={18} strokeWidth={2.25} className="text-red-600" aria-hidden />
        {t.emergencyTitle}
      </h2>
      ...
      <div className="flex flex-wrap gap-2">
        {national.map((c) => (
          ...
          >
            <Phone size={14} strokeWidth={2.25} className="text-blue-700" aria-hidden />
      ...
      {local.length > 0 && (
        <details className="mt-3 group">
          <summary className="flex cursor-pointer list-none items-center gap-1 text-sm font-semibold text-blue-700">
            <ChevronDown size={15} strokeWidth={2.25} className="transition-transform group-open:rotate-180" aria-hidden />
